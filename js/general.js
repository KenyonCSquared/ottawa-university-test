		...
		Form: function() {
			...
			$(document).on('submit', '.affa-form-subscribe.form-general', function() {
				...
				if (pattern.test($email.val()) === false) {
					...
					/*$.ajax({
						type: 'POST',
						url: 'https://taskreminder.notifiapi.com/websitePost',
						data: submitData,
						dataType: 'html',
                        ...
                        var response = JSON.parse(result);
							...
							if (response) {
								...
								if (response['status']==200) {
									...
									$email.removeAttr('disabled').val('Thanks, we got it!').addClass('success');
								} else {
									$submit.removeAttr('disabled');
									$email.removeAttr('disabled').val('Something went wrong').addClass('error');
								}
							}
						}
					});*/
                        ...
                        var response = JSON.parse(result);
							...
							if (response) {
								...
								if (response['status']==200) {
									...
									$submit.removeAttr('disabled');
									$form.find('.submit-status').html('<p class="success"><i class="ion ion-android-checkmark-circle"></i>' + 'We got it!' + '</p>').fadeIn(300).delay(3000).fadeOut(300);
								} else {
									...
									$submit.removeAttr('disabled');
									$form.find('.submit-status').html('<p class="error"><i class="ion ion-close-circled"></i>' + 'Something went wrong :(' + '</p>').fadeIn(300).delay(3000).fadeOut(300);
								}
							}
						}
					});*/
